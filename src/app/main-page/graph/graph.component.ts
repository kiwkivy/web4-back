...
export class GraphComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    console.log(newRFromInputField);
    if (-3 <= parseFloat(newRFromInputField) && parseFloat(newRFromInputField) <= 5) {
  ...
  createTriangle(R: number) {
    ...
    let p2 = this.board.create('point', [-R, 0], {name: '', size: 2, fixed: true});
